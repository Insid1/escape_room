...
const FAKE_BOOKING: IBookingData = {
  name: 'Jon Vek',
  peopleCount: 1,
  phone: '7000000000',
  isLegal: true,
...
const FAKE_NOT_FOUND_MSG = 'Request failed with status code 404';
const FAKE_BAD_REQUEST_MSG = 'Request failed with status code 400';

const fakeNetWorkResponse = () => {
  const mock = new MockAdapter(api);

  mock.onGet('/quests/1').reply(200, FAKE_QUEST);
  mock.onGet('/quests/2').reply(200, FAKE_QUESTS[1]);
  mock.onGet('/quests/').reply(200, FAKE_QUESTS);
  mock.onPost('/orders/').reply(201, {});
};
const fakeNetWorkResponseError = () => {
  const mock = new MockAdapter(api);

  mock.onGet('/quests/500').reply(404, {});
  mock.onGet('/quests/').reply(400, {});
